...
def clean_campaign_data():
    ...
    for zip_file in zip_files:
        with zipfile.ZipFile(zip_file, 'r') as z:
            csv_filename = z.namelist()[0]  # Obtener nombre del archivo dentro del ZIP
            with z.open(csv_filename) as f:
                df = pd.read_csv(f, sep=",", index_col=0)  # Ignorar primera columna vacía
                dfs.append(df)
    ...
    # Debug: Ver nombres de columnas después de limpiar
    data.columns = data.columns.str.strip()  # Limpiar espacios extra
    ...
    # Crear la fecha con formato YYYY-MM-DD
    month_mapping = {
